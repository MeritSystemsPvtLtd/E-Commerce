...
$.extend(shopping_cart, {
	...
	bind_events: function() {
		shopping_cart.bind_shipping_change();
		shopping_cart.payment_mode();
		shopping_cart.cart_shipping_rule();
	...
	bind_change_qty: function() {
		// bind update button
		console.log("In bind_change_qty");
	...
	bind_shipping_change: function() {
			$(".cart-shipping-rule").on("change", ".ship-rule", function() {
					console.log("In bind_shipping_change");
					var newVal = $("#select-ship-rule").val();
					frappe.freeze();
					return frappe.call({
							type: "POST",
							method: "e_commerce.e_commerce.cart_custom.change_in_shipping",
							args: {
									shipp: newVal
							},
	...
	payment_mode: function() {
			console.log("In payment_mode method");
			return frappe.call({
					type: "POST",
					method: "e_commerce.e_commerce.cart_custom.get_payment_mode",
					callback: function(r) {
							if (r.message) {
									$(".cart-payment").html(r.message.payment_mode);
							}
					}
			});
	},

	cart_shipping_rule: function() {
			return frappe.call({
					type: "POST",
					method: "e_commerce.e_commerce.cart_custom.cart_shipping_rule",
					callback: function(r) {
							if (r.message) {
									$(".cart-shipping-rule").html(r.message.cart_shipping_rule);
							}
					}
			});
	},
